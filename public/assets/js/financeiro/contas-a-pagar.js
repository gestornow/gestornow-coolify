...
// Inicialização
// ============================================
function initContasAPagar(config) {
    // Configuração recebida do blade
    window.contasAPagarConfig = config;
...
function excluirConta(id, descricao) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = `/financeiro/contas-a-pagar/${id}`;

            const csrfToken = document.createElement('input');
            csrfToken.type = 'hidden';
            csrfToken.name = '_token';
            csrfToken.value = window.contasAPagarConfig.csrfToken;
...
function toggleParcelas(contaId, idParcelamento) {
    ...
    if (row.style.display === 'none') {
        ...
        icon.classList.add('ti-chevron-up', 'rotated');
        ...
        icon.classList.remove('ti-chevron-up', 'rotated');
...
function carregarParcelas(contaId, idParcelamento) {
    fetch(`/financeiro/contas-a-pagar/parcelas-data/${idParcelamento}`, {
        method: 'GET',
        headers: {
        ...
        .catch(error => {
            ...
            document.getElementById(`parcelas-content-${contaId}`).innerHTML =
                '<div class="alert alert-danger">Erro ao carregar parcelas</div>';
        });
}
...
function renderizarParcelas(contaId, parcelas) {
    ...
    parcelas.forEach(parcela => {
        ...
        html += `<td>
            ...
            ${isParcial ? `<br><small class="text-success">Pago: R$ ${formatMoney(parcela.valor_pago)}</small><br><small class="text-warning">Restante: R$ ${formatMoney(parcela.valor_total - parcela.valor_pago)}</small>` : ''}
        </td>`;
        html += `<td><span class="badge ${statusClass}">${getStatusLabel(parcela.status)}</span></td>`;
        html += `<td>`;
        
        // Botão de histórico se houver pagamentos
        if (parcela.valor_pago > 0) {
            html += `<button type="button" class="btn btn-sm btn-icon btn-outline-info me-1" 
                        onclick="verHistoricoPagamentos(${parcela.id_contas}, '${parcela.descricao}')" 
                        title="Ver pagamentos realizados">
        ...
        // Botão de editar
        html += `<a href="/financeiro/contas-a-pagar/${parcela.id_contas}/edit" 
...
function carregarRecorrencias(contaId, idRecorrencia) {
    fetch(`/financeiro/contas-a-pagar/recorrencias-data/${idRecorrencia}`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
...
function renderizarRecorrencias(contaId, recorrencias) {
    ...
    recorrencias.forEach((recorrencia, index) => {
        ...
        html += `<td>
            ...
            ${isParcial ? `<br><small class="text-success">Pago: R$ ${formatMoney(recorrencia.valor_pago)}</small><br><small class="text-warning">Restante: R$ ${formatMoney(recorrencia.valor_total - recorrencia.valor_pago)}</small>` : ''}
        </td>`;
        ...
        if (recorrencia.valor_pago > 0) {
            html += `<button type="button" class="btn btn-sm btn-icon btn-outline-info me-1" 
                        onclick="verHistoricoPagamentos(${recorrencia.id_contas}, '${recorrencia.descricao}')" 
                        title="Ver pagamentos realizados">
                        <i class="ti ti-receipt"></i>
                    </button>`;
        }
        ...
        // Botão de editar
        html += `<a href="/financeiro/contas-a-pagar/${recorrencia.id_contas}/edit" 
                    class="btn btn-sm btn-icon btn-outline-primary" 
                    title="Editar">
                    <i class="ti ti-edit"></i>
                </a>`;
...
function abrirModalBaixa(idConta, descricao, valorTotal, valorPago = 0) {
    const config = window.contasAPagarConfig;
    ...
    Swal.fire({
        ...
        showCancelButton: true,
        confirmButtonText: '<i class="ti ti-check me-1"></i>Confirmar Baixa',
        cancelButtonText: '<i class="ti ti-x me-1"></i>Cancelar',
        customClass: {
...
function gerarHtmlModalBaixa(descricao, valorTotal, valorRestante, valorPago, config) {
    ...
    if (valorPago > 0) {
        infoHtml += `<br><strong>Já Pago:</strong> R$ ${valorPago.toFixed(2).replace('.', ',')}`;
    ...
    infoHtml += `
            ...
            <div class="mb-3">
                ...
                <small class="text-muted">Se pagar valor menor que o restante, a conta continuará como parcialmente paga</small>
            </div>
            
            <div id="alerta_parcial" class="alert alert-warning mb-3" style="display: none;">
                <i class="ti ti-alert-triangle me-1"></i>
                <strong>Atenção:</strong> Valor a pagar é menor que o restante. A conta continuará como <strong>Parcialmente Paga</strong>.
            </div>
            <div id="alerta_maior" class="alert alert-danger mb-3" style="display: none;">
                <i class="ti ti-alert-circle me-1"></i>
                <strong>Erro:</strong> O valor a pagar não pode ser maior que o valor restante da conta.
            </div>
...
function setupModalBaixa(valorRestante) {
    ...
    // Monitorar mudanças no valor pago para mostrar alerta
...
function validarModalBaixa(valorRestante) {
    ...
    if (valorDecimal > valorRestante) {
        Swal.showValidationMessage(`O valor a pagar (R$ ${valorDecimal.toFixed(2).replace('.', ',')}) não pode ser maior que o valor restante (R$ ${valorRestante.toFixed(2).replace('.', ',')})`);
        return false;
    }

    if (valorDecimal <= 0) {
        Swal.showValidationMessage('O valor pago deve ser maior que zero');
        return false;
    }
...
function darBaixa(idConta, dados) {
    $.ajax({
        url: `/financeiro/contas-a-pagar/${idConta}/dar-baixa`,
        method: 'POST',
        data: dados,
...
function excluirMultiplos(ids, url) {
    $.ajax({
        ...
        method: 'POST',
        data: {
            _token: window.contasAPagarConfig.csrfToken,
            ids: ids
        },
        success: function (response) {
            if (response.success) {
...
// ============================================
function verHistoricoPagamentos(idConta, descricao) {
    $.ajax({
        url: `/financeiro/contas-a-pagar/${idConta}/historico-pagamentos`,
        method: 'GET',
        success: function(response) {
            if (response.success) {
        ...
        error: function(xhr) {
            mostrarErro(xhr.responseJSON?.message || 'Erro ao carregar histórico de pagamentos.');
        }
    });
}
...
function mostrarModalHistorico(descricao, data) {
    ...
    let html = `
        <div class="text-start">
            ...
            <h6 class="mb-2 text-center">Histórico de Pagamentos</h6>
    ...
    if (data.pagamentos.length === 0) {
        html += '<tr><td colspan="6" class="text-center text-muted">Nenhum pagamento registrado</td></tr>';
    } else {
        data.pagamentos.forEach(pagamento => {
            html += `
                <tr>
                    <td>${pagamento.data_pagamento}</td>
                    <td><strong class="text-success">R$ ${formatMoney(pagamento.valor_pago)}</strong></td>
                    <td>${pagamento.forma_pagamento}</td>
                    <td>${pagamento.banco}</td>
                    <td>
                        <small>${pagamento.usuario}</small><br>
                        <small class="text-muted">${pagamento.created_at}</small>
                    </td>
                    <td class="text-center">
                        <button type="button" class="btn btn-sm btn-icon btn-outline-danger" 
                                onclick="excluirPagamentoParcial(${data.id_conta}, ${pagamento.id})" 
    ...
    Swal.fire({
        title: '<div class="text-center">Histórico de Pagamentos</div>',
        html: html,
...
function verLogAtividadesConta(idConta, descricao) {
    $.ajax({
        url: `/financeiro/contas-a-pagar/${idConta}/logs-atividades`,
        method: 'GET',
        success: function (response) {
            if (response.success) {
...
function gerarResumoContextoLog(contexto) {
    ...
    const temDadosBaixa = contexto.valor_baixa !== undefined
        || contexto.valor_pago_antes !== undefined
        || contexto.valor_pago_depois !== undefined
        || contexto.valor_restante_depois !== undefined;
    ...
    const linhas = [
        ...
        { label: 'Valor da baixa', valor: formatarMoedaLog(contexto.valor_baixa) },
        ...
        { label: 'Restante', valor: formatarMoedaLog(contexto.valor_restante_depois) },
        { label: '% quitado', valor: contexto.percentual_quitado !== undefined ? `${Number(contexto.percentual_quitado).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}%` : '-' },
...
function resumirObjeto(obj) {
    ...
    const preview = entries.slice(0, 4).map(([chave, valor]) => {
        const valorTexto = valor === null || valor === undefined || valor === '' ? '-' : String(valor);
        return `${escapeHtml(chave)}: ${escapeHtml(valorTexto)}`;
    });

    if (entries.length > 4) {
        preview.push(`... +${entries.length - 4} alteração(ões)`);
    }
...
// Função para excluir pagamento parcial
function excluirPagamentoParcial(idConta, idPagamento) {
    Swal.fire({
        title: 'Confirmar exclusão',
        text: 'Deseja realmente excluir este pagamento? O valor será deduzido do total pago da conta.',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#8592a3',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: `/financeiro/contas-a-pagar/${idConta}/pagamentos/${idPagamento}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': token,
                    'Accept': 'application/json',
                ...
                error: function(xhr) {
                    if (xhr.status === 419 || xhr.status === 401) {
                        ...
                        mostrarErro(xhr.responseJSON?.message || 'Erro ao excluir pagamento.');
                    }
                }
            });
        }
...
// ============================================
window.initContasAPagar = initContasAPagar;
