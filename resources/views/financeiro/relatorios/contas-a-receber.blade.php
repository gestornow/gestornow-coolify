...
<div class="container-xxl flex-grow-1">
    <div class="card">
        ...
        <div class="card-body">
            <form id="formRelatorio" method="GET" action="{{ route('financeiro.relatorios.contas-receber.gerar') }}" target="_blank">
                <div class="row g-3">
                    <!-- Período -->
                    <div class="col-12">
                    ...
                    <div class="col-md-3">
                        <label class="form-label" for="tipo_data">Tipo de Data</label>
...
async function gerarRelatorioPDF() {
    ...
    try {
        ...
        const response = await fetch("{{ route('financeiro.relatorios.contas-receber.pdf') }}", {
            method: 'POST',
            headers: {
        ...
        await gerarPDFContasReceber(dados);
...
async function gerarRelatorioExcel() {
    ...
    try {
        // Buscar dados via AJAX
        const response = await fetch("{{ route('financeiro.relatorios.contas-receber.excel') }}", {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/x-www-form-urlencoded',
        ...
        await gerarExcelContasReceber(dados);
