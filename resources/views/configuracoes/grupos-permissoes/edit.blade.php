...
@section('title', 'Editar Grupo de Permissoes')
    ...
    <div class="card mb-4">
        <div class="card-header border-bottom-0 pb-0">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <h5 class="mb-0">Editar Grupo de Permissoes</h5>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('configuracoes.grupos-permissoes.update', $grupo->id_grupo) }}" id="formGrupoPermissoes">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3 mb-4">
                    <div class="col-12 col-lg-7">
                        ...
                        <input type="text" name="nome" class="form-control @error('nome') is-invalid @enderror" value="{{ old('nome', $grupo->nome) }}" required>
                        @error('nome')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="col-12 col-lg-5">
                        <label class="form-label">Descricao</label>
                        <input type="text" name="descricao" class="form-control @error('descricao') is-invalid @enderror" value="{{ old('descricao', $grupo->descricao) }}">
                    ...
                    <div class="col-12 col-md-4">
                        <div class="card h-100 border-info border-opacity-25">
                            <div class="card-body">
                                ...
                                <h4 class="mb-0" id="totalSelecionadas">{{ count(old('chaves', $chavesSelecionadas)) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="perm-module-card mb-3" data-module-card>
                        ...
                        <div class="p-3">
                            ...
                            @foreach($chaves as $item)
                                @php
                                    $checked = in_array($item->chave, old('chaves', $chavesSelecionadas), true);
                                @endphp
                                <div class="col-12 col-md-6 col-xl-4" data-perm-item data-search="{{ strtolower($item->label . ' ' . $item->chave) }}">
                                    <label class="perm-item d-flex align-items-start gap-2" for="chk_{{ md5('edit_' . $item->chave) }}">
                                        <input class="form-check-input mt-1" type="checkbox" name="chaves[]" value="{{ $item->chave }}" id="chk_{{ md5('edit_' . $item->chave) }}" {{ $checked ? 'checked' : '' }}>
                                        <span>
